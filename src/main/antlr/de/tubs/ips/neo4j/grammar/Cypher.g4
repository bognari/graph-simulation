...
readOnlyEnd : readPart return1 ;

readUpdateEnd : readingClause ( SP? readingClause )* ( SP? updatingClause )+ ( SP? return1 )? ;

updatingEnd : updatingStartClause ( SP? updatingClause )* ( SP? return1 )? ;
...
return1 : RETURN ( SP? DISTINCT )? SP returnBody ;
